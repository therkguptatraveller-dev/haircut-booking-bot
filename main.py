...
async def my_shops(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = "Aapki shops:\n"
    for shop in shops:
        status = shop['status'].capitalize() if shop['status'] else 'Unknown'
        text += f"- {shop['shop_name']} ({shop['city']}) - Status: {status}\n"
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not data.startswith(("approve_shop_", "reject_shop_")):
        await query.message.edit_text("Invalid action!")
        return

    shop_id = int(data.split("_")[2])
    logger.info(f"Admin action on shop {shop_id}: {data}")

    conn = get_db_connection()
    cursor = conn.cursor()

    new_status = 'approved' if data.startswith("approve_shop_") else 'rejected'
    cursor.execute("UPDATE shops SET status = ? WHERE id = ?", (new_status, shop_id))
    conn.commit()
    ...
    if owner_id:
        try:
            await context.bot.send_message(
                chat_id=owner_id,
                text=f"🎉 Badhai ho! Aapki shop ID {shop_id} {status_text}\nAb aapki shop live hai users ke liye!"
            )
            logger.info(f"Notification sent to owner {owner_id} for shop {shop_id}")
        except Exception as e:
            logger.error(f"Failed to send notification to owner {owner_id}: {e}")
    else:
        logger.warning(f"Owner ID not found for shop {shop_id}")

    # Admin message edit
    await query.message.edit_text(f"Shop ID {shop_id} {status_text}")
